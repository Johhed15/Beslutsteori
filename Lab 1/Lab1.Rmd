...
These calculations needs to be scaled to be able to get the exponent for the logged values, this can be done by subtracting with the largest likelihood as calculating exp on large negative values is not a computational problem.
...
Same results as in the question and as the first method, but this is the preferred way to solve the question as its a much simpler.
...
My subjective odds is that the odds against that Little Joe will win is 17 to 3 and the odds for Little Joe is then 0.1764706.
